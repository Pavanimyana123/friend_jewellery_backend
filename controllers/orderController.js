    ...
    orderData.balance_amt || 0, orderData.net_wt || 0, orderData.summary_price || 0, orderData.summary_rate || 0, orderData.receipt_amt || 0, actual_order_id
            ...
            return new Promise((resolve, reject) => {
              ...
              UPDATE orders SET 
                ...
                overall_total_price=?, overall_stone_price=?, overall_total_mc=?, overall_tax_amt=?, advance_gross_wt=?, fine_wt=?, advance_finewt_amt=?, advance_amount=?, balance_amt=?, net_wt=?,
                summary_price=?, summary_rate=?, receipt_amt=?
              WHERE actual_order_id=?`;

                        const updateValues = [...values];
                        updateValues.splice(63, 1); // remove duplicate actual_order_id in SET
        ...
        if (isNewOrderInserted && firstOrderData && firstOrderData.mobile && firstOrderData.order_number) {
            try {
                ...
                const orderConfirmationFinalUrl = `${orderConfirmationUrl}?${Object.entries(orderConfirmationParams)
                    .map(([k, v]) => `${k}=${v}`)
                    .join("&")}`;

                // 2. Send APK Download SMS
                const apkDownloadText = "Dear Customer, Please click on the link below to download the APK to know your order details : https://drive.google.com/file/d/1oVzEOoTlWQnQpZB1-iln3NjbwjXw-Ybn/view?usp=drive_link - NEW FRIENDS JEWELLERY";

                const apkDownloadParams = {
                    username: process.env.SMS_USERNAME || "friendsjewellers",
                    pass: process.env.SMS_PASSWORD || "user@123",
                    senderid: process.env.SMS_SENDERID || "FRNJEW",
                    dest_mobileno: firstOrderData.mobile,
                    message: encodeURIComponent(apkDownloadText),
                    dltentityid: process.env.SMS_ENTITYID || "1101383680000087435",
                    dlttempid: SMS_CONFIG.templateIds.app_download || "1107176139706909553",
                    response: "Y",
                };

                const apkDownloadFinalUrl = `${orderConfirmationUrl}?${Object.entries(apkDownloadParams)
                    .map(([k, v]) => `${k}=${v}`)
                    .join("&")}`;

                // Send both SMS messages
                await Promise.all([
                    axios.get(orderConfirmationFinalUrl),
                    axios.get(apkDownloadFinalUrl)
                ]);

                // console.log("✅ Both order confirmation and APK download SMS sent successfully:", firstOrderData.mobile);
            } catch (smsError) {
